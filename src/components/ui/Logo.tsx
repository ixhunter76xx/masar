 ...
 * ملفٌّ واحد شفّاف الخلفية، يُشتقّ آليًّا من مصدر الهوية:
 *
 *   npx tsx --tsconfig tsconfig.script.json scripts/generate-icons.mts assets/logo-source.png
 *
 * ولا يُحرَّر بيدٍ: `brand-regression` يقارن بصمته ببصمة بقيّة الأصول،
 * فيمسك الملفّ الذي بُدِّل وحده أو تُرك وحده.
 *
 * والنسخة ملحقةٌ بالعنوان لا مكتوبةً هنا: تخزين المتصفّح المؤقّت يُبقي
 * الشعار القديم حيًّا حتى بعد النشر، ما لم يتغيّر العنوان نفسه.
 * ─────────────────────────────────────────────────────────────────────
 */
const SRC = `/logo-masar.png?v=${SITE.brandVersion}`;

const SOURCES: Record<Exclude<LogoVariant, "auto">, string> = {
  full: SRC,
  /* المرجع المعتمد يقدّم هويةً واحدة لا رمزًا بديلًا. */
  mark: SRC,
};
