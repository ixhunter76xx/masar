...
/**
 * مرشّح Prisma: «هذا المستخدم يملك منتجًا في هذا المقرر» — نطاق المقرر.
 *
 * ── لماذا دالة مسمّاة بدل تكرار الشرط ───────────────────────────────
 * كان الشرط نفسه مكتوبًا حرفيًا في اثني عشر موضعًا، بعضها صحيح عمدًا
 * وبعضها بقيّة من زمن ما قبل الحزم. من يقرأ أحدها لا يستطيع أن يعرف
 * أيّهما هو — فيُصلح ما لا يحتاج إصلاحًا، أو يترك ما يحتاجه.
 *
 * **الاسم يعلن النية**: ما يستدعي هذه الدالة نطاقه المقرر بقرار، وهي
 * الميزات التي لا تُباع بالحزمة: الإعلانات والرسائل والدرجات وسجل
 * النشاط وقائمة «مقرراتي». ما نطاقه الحزمة يستدعي `canViewLesson`
 * أو `canViewQuiz` أو `canViewAssignment` — ولا يستدعي هذه أبدًا.
 *
 * ── ويشمل `notExpired()` ────────────────────────────────────────────
 * المواضع الاثنا عشر كانت تتجاهل الانتهاء، فمن استُرجع طلبه — والاسترجاع
 * يضبط `expiresAt` — كان يبقى يرى إعلانات المقرر ورسائله ودرجاته
 * ويجده في «مقرراتي». الآن يسقط منها جميعًا بالضبط كما يسقط من الدروس.
 */
export function enrolledInCourse(userId: string) {
  return {
    products: { some: { enrollments: { some: { userId, ...notExpired() } } } },
  };
}
